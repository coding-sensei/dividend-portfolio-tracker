...
function is_transaction_a_dividend(transaction) {
    if(transaction.get("Action").includes("Div")) {
        ...
        var columns_to_check = ["Details", "Quantity", "Price", "Fees & Comm"]
...
function get_total_dividends_from_transactions(transactions) {
    ...
    for (const transaction of transactions) {
        ...
        if(is_transaction_a_dividend(transaction)) {
            var msg = "Adding " + transaction.get("Amount") + " from " + transaction.get("Description*");
            console.log(msg);
...
function add_calc_button(function_call_on_click) {
    let searchObj = document.getElementsByClassName("sdps-button sdps-button--small sdps-button--primary thrd-searchbtn")[0];
    let btn = document.createElement("button");
    btn.innerHTML = "Calculate Dividends";
    btn.className = "sdps-button sdps-button--small sdps-button--primary thrd-searchbtn";
    btn.onclick = () => {
...
waitForKeyElements (
    "#midpane > div.sdps-layout__main-app > app-root > div > app-transactions-history-component > div.transaction-page-container > div:nth-child(2) > div.transaction-grid-container",
    create_calc_button,
    false
);
